...
EXCLUDE='not fails_on_rgw and not fails_on_aws and not fails_on_dbstore and not s3select and not sns and not cloud_transition and not cloud_restore and not sse_s3 and not encryption and not delete_marker and not lifecycle_transition and not s3website and not bucket_logging and not bucket_policy and not object_ownership and not conditional_write and not group and not user_policy and not role_policy and not session_policy and not iam_account and not iam_role and not iam_user and not webidentity_test'
...
KEYWORDS='not acl and not access_bucket and not post_object and not anon and not _raw_ and not policy and not public_block and not torrent and not aws_chunked and not chunked_transfer and not multipart_copy and not copy_not_owned and not 100_continue and not expected_bucket_owner and not website'
