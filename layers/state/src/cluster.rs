...
impl ClusterDb {
    ...
    pub async fn connect(pd_endpoints: &[&str]) -> Result<Self, StateError> {
        let endpoints: Vec<String> = pd_endpoints.iter().map(|s| s.to_string()).collect();
        let client = RawClient::new(endpoints)
            .await
            .map_err(|e| StateError::Database(format!("TiKV connect failed: {e}")))?;
    ...
    pub async fn put<T: Serialize>(
        ...
        self.client
            ...
            .await
            .map_err(|e| StateError::Database(format!("TiKV put failed: {e}")))?;
        Ok(())
    }
    ...
    pub async fn get<T: DeserializeOwned>(
        ...
        let value = self
            ...
            .await
            .map_err(|e| StateError::Database(format!("TiKV get failed: {e}")))?;
    ...
    pub async fn delete(&self, namespace: &str, key: &str) -> Result<(), StateError> {
        ...
        self.client
            ...
            .await
            .map_err(|e| StateError::Database(format!("TiKV delete failed: {e}")))?;
        Ok(())
    }
    ...
    pub async fn list<T: DeserializeOwned>(
        ...
        let pairs = self
            ...
            .await
            .map_err(|e| StateError::Database(format!("TiKV scan failed: {e}")))?;
    ...
    pub async fn batch_put<T: Serialize>(
        ...
        self.client
            .batch_put(kvs)
            .await
            .map_err(|e| StateError::Database(format!("TiKV batch_put failed: {e}")))?;
