...
// Mirror of the Raft-replicated portion of the `hypervisor` resource
// (generated by `#[resource]` in `layers/hypervisor/src/definition.rs`).
// Kept inline to avoid the state crate reaching across layer boundaries;
// if the Hypervisor struct changes, update here too — the point of this
// test is to catch non-deterministic defaults being reintroduced on
...
DEFINE FIELD IF NOT EXISTS created_at  ON hypervisor TYPE datetime;\
DEFINE FIELD IF NOT EXISTS updated_at  ON hypervisor TYPE datetime;\
DEFINE FIELD IF NOT EXISTS version     ON hypervisor TYPE int;\
DEFINE INDEX IF NOT EXISTS hypervisor_public_key ON hypervisor FIELDS public_key UNIQUE;\
DEFINE INDEX IF NOT EXISTS hypervisor_node_id ON hypervisor FIELDS node_id UNIQUE;\
DEFINE INDEX IF NOT EXISTS hypervisor_raft_addr ON hypervisor FIELDS raft_addr UNIQUE;\
";
        ...
        let rows: Vec<HypervisorRow> = db
            .query_take(&format!(
                "SELECT public_key, node_id, address, endpoint, allowed_ips, \
                 keepalive, raft_addr, created_at, updated_at, version \
...
async fn hypervisor_write_is_byte_identical_across_nodes() {
    ...
    // Caller formats timestamps up front so every node's apply writes the
    // same literal — the point of the fix.
    let ts = "2026-04-17T12:34:56.123456789Z";
    let surql = format!(
        "CREATE hypervisor SET \
         public_key = 'pk-deterministic', node_id = 42, \
         raft_addr = '[fd00::1]:4001', address = 'fd00::1', \
         endpoint = '203.0.113.5:51820', allowed_ips = ['fd00::1', 'fd00::2'], \
         keepalive = 25, \
         created_at = d'{ts}', updated_at = d'{ts}', version = 0"
    );
    ...
    let expected: Datetime = ts.parse().expect("parse expected timestamp");
    assert_eq!(row_n1.created_at, expected, "created_at changed on apply");
    assert_eq!(row_n1.updated_at, expected, "updated_at changed on apply");
}
...
async fn late_joiner_snapshot_install_matches_leader() {
    ...
    for i in 0..write_count {
        ...
        let surql = format!(
            "CREATE hypervisor SET \
             public_key = 'pk-snap-{i}', node_id = {i}, \
             raft_addr = '[fd00::{i:x}]:4001', address = 'fd00::{i:x}', \
    ...
    n4.db
        .query(
            "CREATE hypervisor SET \
             public_key = 'pk-snap-0', node_id = 999, \
             raft_addr = '[fd00::pre]:4001', address = 'fd00::pre', \
             endpoint = NONE, allowed_ips = ['fd00::pre'], keepalive = 25, \
             created_at = d'2020-01-01T00:00:00.000000000Z', \
             updated_at = d'2020-01-01T00:00:00.000000000Z', version = 0",
        )
        .await
