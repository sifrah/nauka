...
impl super::Reconciler for StoreReconciler {
    ...
    async fn reconcile(&self, ctx: &ReconcileContext) -> anyhow::Result<ReconcileResult> {
        ...
        result.desired = stores
            .iter()
            .filter(|s| is_store_eligible_for_deregister(&s.state_name))
            .count();

        // For each "Up" store whose address matches an unreachable peer, offline it
        for store in &stores {
            if !is_store_eligible_for_deregister(&store.state_name) || store.id == 0 {
                continue;
            }

            if !match_store_to_peer(&store.address, &unreachable_addrs) {
                continue;
            }
